...
const validateQuarter = (quarter: any): boolean => {
  return Boolean(
    quarter &&
    quarter.samples &&
    Array.isArray(quarter.samples) &&
    quarter.samples.length === 4 &&
...
const QuarterManagement: React.FC = () => {
  ...
  const handleQuarterSubmit = async (quarterData: any) => {
    try {
      if (!validateQuarter(quarterData)) {
      ...
      console.error('Quarter submission failed:', error);
      toast({
        title: 'Error',
        description: 'Failed to create quarter. Please check the data and try again.',
        type: 'error'
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await handleQuarterSubmit(quarterData);
    } catch (err) {
